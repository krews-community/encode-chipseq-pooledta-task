...
class TestCmdRunner : CmdRunner {
    override fun run(cmd: String) = exec("docker", "exec", "chipseq-pooledta-base", "sh", "-c", cmd)
    override fun runCommand(cmd: String):String? = getCommandOutput("docker", "exec", "chipseq-pooledta-base", "sh", "-c", cmd)
